...
- `provider.go`: provider registry (OpenAI, DeepSeek) — endpoints, API key env vars, web-search tool types, default models.
...
- Run CLI: `./bin/answer -q "Latest AI developments" -model gpt-5.4-mini`.
...
- Require an API key via environment or `.env`: `OPENAI_API_KEY` (default) or `DEEPSEEK_API_KEY` (with `PROVIDER=deepseek` / `-provider deepseek`); never log or commit secrets.
